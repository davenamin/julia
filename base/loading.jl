...
function find_unsuitable_manifests_versions()
    ...
    for env in load_path()
        ...
        @label begin
            man_julia_version isa String || break
            man_julia_version = VersionNumber(man_julia_version)
            thispatch(man_julia_version) != thispatch(VERSION) && break
            isempty(man_julia_version.prerelease) != isempty(VERSION.prerelease) && break
            isempty(man_julia_version.prerelease) && continue
            man_julia_version.prerelease[1] != VERSION.prerelease[1] && break
