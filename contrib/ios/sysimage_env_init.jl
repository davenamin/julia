...
# overwrites these globals at startup before anything reads them.  It must,
# however, be called only ONCE per process: a second call wraps the same
# fd 0/1/2 libuv handles in a fresh set of stream objects, and when GC finalizes
# the orphaned first set, `uvfinalize` calls `jl_close_uv` on the *shared*
# handle — after which every write to stdout/stderr fails with `EBADF`.
# `generate_precompile.jl` calls `reinit_stdio()` too, but for extra-package
# bakes it is invoked with arg `0` (see sysimage-ios.mk), which makes it skip
# that call (and all statement collection), so this is the only one.
Base.reinit_stdio()
